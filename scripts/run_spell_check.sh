...
# Vendored/external content (e.g. third-party skill docs under .agents/skills/,
# symlinked from .claude/skills/) is excluded: it isn't authored by this
# project and isn't expected to match the custom en_US dictionary.
...
done < <(git ls-files -co --exclude-standard -z -- "*.md" "*.html" "*.astro" ":(exclude).agents/skills/**" ":(exclude).claude/skills/**")
